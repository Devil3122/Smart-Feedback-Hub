...
export default function Dashboard() {
  ...
  const handleAuditAction = async (historyId: number, action: "Approve" | "Reject") => {
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || `Failed to ${action.toLowerCase()} request`);
      }
    } catch (e) {
      console.error(e);
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">View and play feedback submitted by this user</p>
                  </div>
                  <Button variant="outline" onClick={() => setSelectedUserFeedback(null)}>
                    Back to User List
                  </Button>
                </div>
                ...
                <Card className="shadow-sm border-slate-200">
                  <CardContent className="pt-6">
                    {stats.recentFeedbacks.filter((fb: any) => fb.userId === selectedUserFeedback.id).length === 0 ? (
                      <p className="text-slate-500 text-center py-8">No feedback submitted by this user.</p>
                    ) : (
                      <div className="overflow-x-auto border rounded-lg">
                        <table className="w-full text-sm text-left">
                          <thead className="bg-slate-50 dark:bg-slate-900 text-xs uppercase text-slate-700 dark:text-slate-300">
                          ...
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {stats.recentFeedbacks
                              .filter((fb: any) => fb.userId === selectedUserFeedback.id)
                              .map((fb: any) => (
                                <tr key={fb.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20">
                ...
                {auditLogs.length === 0 ? (
                  ...
                  <div className="space-y-6">
                    {auditLogs.map((log: any) => (
                      <div key={log.id} className="border border-slate-200 rounded-lg p-4 bg-slate-50 relative space-y-4">
                        <div className="absolute top-4 right-4 text-xs text-slate-400">
                          {format(new Date(log.modifiedAt), "MMM d, yyyy h:mm a")}
                        </div>
                        <h4 className="font-semibold text-slate-700">Original Feedback ID: {log.originalFeedbackId}</h4>
                        
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="p-3 bg-red-50/50 border border-red-100 rounded text-sm text-red-900">
                            <div className="font-bold text-xs uppercase text-red-500 mb-1">Previous/Current State:</div>
                            <p className="italic">"{log.previousText}"</p>
                            {log.currentRating !== undefined && log.currentRating !== null && (
                              <div className="mt-2 text-xs font-semibold text-red-700">Rating: {log.currentRating} / 5 ★</div>
                            )}
                            {log.previousAudioUrl && <div className="mt-2 text-xs text-red-600">[Audio Attached]</div>}
                          </div>

                          <div className="p-3 bg-green-50/50 border border-green-100 rounded text-sm text-green-900">
                            <div className="font-bold text-xs uppercase text-green-500 mb-1">Proposed Edits:</div>
                            <p className="italic">"{log.proposedText || log.previousText}"</p>
                            {log.proposedRating !== undefined && log.proposedRating !== null && (
                              <div className="mt-2 text-xs font-semibold text-green-700">Proposed Rating: {log.proposedRating} / 5 ★</div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex justify-between items-center pt-2 border-t border-slate-200">
                          <div className="text-sm">
                            ...
                            <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                              log.reviewStatus === 'Pending Approval' ? 'bg-amber-100 text-amber-800' :
                              log.reviewStatus === 'Approved' ? 'bg-green-100 text-green-800' :
                              log.reviewStatus === 'Rejected' ? 'bg-red-100 text-red-800' : 'bg-slate-100 text-slate-800'
                            }`}>
                              {log.reviewStatus}
                            </span>
                          </div>

                          {log.reviewStatus === "Pending Approval" && (
