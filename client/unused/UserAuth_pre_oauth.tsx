...
export default function UserAuth() {
  ...
  const handleLogin = async () => {
    // Attempt standard user login only
    ...
    if (response.ok) {
      ...
      throw new Error(errData.error || "DATABASE_CONNECTIVITY_ERROR");
    }
                  ...
                  <div className="relative">
                    ...
                    {mode === "register" && username && (
                      <div className="absolute right-3 top-3">
                        ...
                        {usernameAvailable === false && <XCircle className="w-4 h-4 text-red-750" />}
                      </div>
                    )}
                  </div>
                </div>
