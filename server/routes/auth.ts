...
export const authenticateToken: RequestHandler = (req, res, next) => {
  ...
  try {
    const userPayload = jwt.verify(token, process.env.JWT_SECRET || 'super-secret-key');
...
export const loginAdmin: RequestHandler = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user.id, username: user.username, role: user.role }, process.env.JWT_SECRET || 'super-secret-key', { expiresIn: '7d' });
