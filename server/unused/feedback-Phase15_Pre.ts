...
export const editFeedback: RequestHandler = async (req, res) => {
  try {
    const { id } = req.params;
    const { textContent, rating, userId } = req.body;
...
export const adminOverride: RequestHandler = async (req, res) => {
  try {
    const { historyId, action } = req.body; // 'Revert to Old', 'Accept Modified', 'Save Both'
    ...
    if (action === "Revert to Old") {
      originalFeedback.textContent = historyEntry.previousText;
      originalFeedback.audioUrl = historyEntry.previousAudioUrl;
      originalFeedback.updatedAt = new Date().toISOString();
      historyEntry.reviewStatus = "Reverted";
    } else if (action === "Accept Modified") {
      historyEntry.reviewStatus = "Accepted";
    } else if (action === "Save Both") {
      const previousRating = historyEntry.currentRating ?? originalFeedback.rating;
      ...
      if (typeof db.update === "function") {
        try {
          ...
          console.warn("Drizzle database update failed:", err);
        }
      }
      ...
      if (typeof db.update === "function") {
        try {
          ...
          console.warn("Drizzle history update failed:", err);
        }
      }
    } else {
